...
public class ZenMoneyClient
{
   ...
   /*public void getInitialData(Callback callback) throws JSONException
   {
    ...
    public @Nullable UUID sendTransactions(@NonNull Transaction transaction)
    {
        UUID tag = null;
        Log.i(this.getClass().getName(), "[sendTransactions] ");
        try
        {
            RequestBody body = RequestBody.create(JSON, RequestUtils.getNewTransactionRequestBody(transaction));
            tag = doRequest(body, new InternalCallback(handler));
        } catch (Exception e)
        {
            handler.processError(e);
    ...
    public @Nullable UUID getAccounts()
    {
        UUID tag = null;
        try
        {
            Log.i(this.getClass().getName(), "[getAccounts] ");
            RequestBody body = RequestBody.create(JSON, RequestUtils.getAccountsRequestBody());
            tag = doRequest(body, new InternalCallback(handler));
        } catch (Exception e)
        {
            handler.processError(e);
